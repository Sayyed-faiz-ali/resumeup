...
import Alert from './aleart'; // Your Alert component
...
function SignUp() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${API_URL}/api/auth/register`, formData); // Updated URL
  ...
  return (
    <div
      className="bg-gray-900 min-h-screen flex items-center justify-center font-sans text-white bg-cover bg-center relative"
      style={{ backgroundImage: "url('/lo.jpg')" }}
    >
      {alert && <Alert message={alert.message} type={alert.type} onClose={closeAlert} />}

      <div className="relative rounded-xl shadow-lg p-10 max-w-sm w-full text-center backdrop-blur-md">
        ...
        <form autoComplete="on" onSubmit={handleSubmit}>
          <input
            type="text" name="name" value={name} onChange={handleInputChange} placeholder="Name"
          ...
          />
          <input
            type="email" name="email" value={email} onChange={handleInputChange} placeholder="Email"
            className="w-full p-4 mb-5 rounded-lg border border-gray-700 bg-gray-700 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
            required
          />
          <input
            type="password" name="password" value={password} onChange={handleInputChange} placeholder="Password"
            className="w-full p-4 mb-5 rounded-lg border border-gray-700 bg-gray-700 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
            required
          />
          <button
